...
object EvaluatorSpec extends Specification with ScalaCheck {
  ...
  "Evaluator" should {
    "failed with DivideByZero abend" in {
      List("PUSH (1/0)",
        ...
        code:String =>
          new Evaluator().evaluate(parse(code).get, new Controller(State(2, 2, 0))) == EvaluationResult(None, Some(DivideByZero))
        }
    }
  }
}