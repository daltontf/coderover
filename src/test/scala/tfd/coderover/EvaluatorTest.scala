...
class EvaluatorTest extends TestCase {
  ...
  def testObstructed() {
    ...
    evaluator.evaluate(parse("IF NOT(OBSTRUCTED(X+DX,Y+DY)) { FORWARD } ELSE { RIGHT }").get, state)
    ...
    evaluator.evaluate(parse("RIGHT IF NOT(OBSTRUCTED(X+DX,Y+DY)) { FORWARD } ELSE { RIGHT }").get, state)
    assertEquals(State(0,1,2), state)
    // Test outside grid boundaries
    evaluator.evaluate(parse("IF (OBSTRUCTED(-1,0)) { RIGHT }").get, state)
    assertEquals(State(0,1,3), state)
    evaluator.evaluate(parse("IF (OBSTRUCTED(0,-1)) { RIGHT }").get, state)
    assertEquals(State(0,1,0), state)
    evaluator.evaluate(parse("IF (OBSTRUCTED(0,0)) { RIGHT }").get, state)
    assertEquals(State(0,1,0), state)
    evaluator.evaluate(parse("IF (OBSTRUCTED(2,3)) { RIGHT }").get, state)
    assertEquals(State(0,1,1), state)
    evaluator.evaluate(parse("IF (OBSTRUCTED(3,2)) { RIGHT }").get, state)
    assertEquals(State(0,1,2), state)
    evaluator.evaluate(parse("IF (OBSTRUCTED(2,2)) { RIGHT }").get, state)
    assertEquals(State(0,1,2), state)
  }
